...
bool isNumber(string s)
{
    ...
    while (it != s.end() && std::isdigit(static_cast<unsigned char>(*it), loc)) ++it;
			...
			if(isNumber(uinput)) // if the number was valid
			{
				...
				if(uinput == "1")
				{
					...
					if(numberWasFound) // need to convert its type accordingly
					{
		    			...
		    			string intPriorityStr = (string)temp; 
				  		...
				  		int digits = floor(logBase10(num)) + 1;
				...
				else if(uinput == "3")
				{
					...
					else // valid number input
					{
						...
						arr.insert(arr.begin()+itemNum+1, temporary); // itemNum = 1 pos = 3
						if(itemNum+1 < pos) // if request is to insert before item, item number will be +1
